...
export default {
  ...
  computed: {
    ...
    style () {
      return `width:${this.width / this.ratio}px;height:${this.height / this.ratio}px;${this.rotate};${this.opacity}`
    },
    rotate () {
      let deg = Math.floor((this.pullDownTop + 50) / 60 * 585)
      return `transform:rotate(${deg}deg)`
    },
    opacity () {
      let opacity = (this.pullDownTop + 50) / 60
  ...
  methods: {
    ...
    _drawArrow (ctx) {
      ...
      ctx.fillStyle = 'rgb(211,58,49)'
      ctx.fill()
      ctx.strokeStyle = 'rgb(211,58,49)'
      ctx.stroke()
      ctx.restore()
    }
  },
